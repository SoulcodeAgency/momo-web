...
export function CanvasPrice({ context, component }: Props) {
  ...
  const uniformCurrency = <UniformText context={context} component={component} parameterId="currency" placeholder="$" />;
  const uniformDescription = <UniformText context={context} component={component} parameterId="description" placeholder="per user / month" />;
  return (
    <Price price={uniformPrice} description={uniformDescription} currency={uniformCurrency} />
  );
}
